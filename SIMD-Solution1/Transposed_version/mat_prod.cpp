...
int main()
{
     ...
     dtime = omp_get_wtime();
     transpMat(B.get(), BT.get(), dim);
     dtime = omp_get_wtime() - dtime;
     std::cout << "Transposition occurred in " << dtime << " s." << std::endl
               << "------------------------------------------------------------" << std::endl;

     dtime = omp_get_wtime();
     multMat_T(A.get(), BT.get(), C.get(), dim);
     dtime = omp_get_wtime() - dtime;
     std::cout << "No transposed, no OMP" << std::endl
               << "CPU time for multiplication: " << dtime << " s       Mflops/sec = " << Mflops / dtime << std::endl
     ...
     dtime = omp_get_wtime();
     multMat_T_omp(A.get(), BT.get(), C2.get(), dim);
     dtime = omp_get_wtime() - dtime;
     ...
     std::cout << "---------------------------------" << std::endl
               << "Difference CPU-CPUOMP:              " << check << std::endl
               << "---------------------------------" << std::endl;
